...
(function(){
  ...
  function specCompareTable(models, specRows, specConst, specNote){
    specRows = specRows || SPECROWS; specConst = specConst || SPECCONST;
    var withData=[], noData=[];
    models.forEach(function(m){ var s=SPECS[m.key!=null?m.key:specKey(m.name)]; if(s){withData.push({label:m.label,s:s});} else {noData.push(m.label);} });
    var note = noData.length ? '<div class="ns-cmp-note">'+noData.join(', ')+' - key figures available on request.</div>' : '';
    if(specNote) note += '<div class="ns-cmp-note">'+specNote+'</div>';
    if(!withData.length){
  ...
  function groupInfo(g, rows){
    ...
    /* g.specModels: explicit spec columns [{label,key}] - for variants that have figures but no catalogue row (23 Sep 2026) */
    var smodels = g.specModels || C.specModels;
    if(smodels) models = smodels.map(function(m){ return {label:m.label, name:m.label, key:m.key}; });
    var specCmp = specCompareTable(models, g.specRows, g.specConst, g.specNote || C.specNote);
  ...
  if(mainHost){
    ...
    if(groupsCfg.length >= 2 && groupEls.length >= 2){
      ...
      bar.innerHTML =
        ...
        '<span class="ns-search-ico" aria-hidden="true"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><line x1="20" y1="20" x2="16.65" y2="16.65"/></svg></span>'+
