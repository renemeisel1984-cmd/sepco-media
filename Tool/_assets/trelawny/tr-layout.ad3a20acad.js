...
(function(){
  ...
  function keyTiles(card, gid){
    ...
    return tiles.length>=2 ? '<div class="ns-spot-keys'+(tiles.length===4?' n4':'')+'">'+tiles.join('')+'</div>' : '';
  }
  ...
  '.ns-spot-keys.n4{grid-template-columns:repeat(2,minmax(0,1fr))}'+'@media(min-width:1500px){.ns-spot-keys.n4{grid-template-columns:repeat(4,minmax(0,1fr))}}'+
  ...
  /* the lightbox's "Quick view" pill would cover a 72-96 px row photo: icon only */
  '.ns-list .ns-thumb .lb-btn::after{content:"\\2922";margin:3px;padding:2px 6px;font-size:11px}'+
